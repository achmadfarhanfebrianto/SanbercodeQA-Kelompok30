...
//Edit job title with special characters in all field
describe('failed_editjobtitle', () => {
    it('failed_editjobtitle', () => {
      ...
      cy.get(':nth-child(2) > .oxd-input').clear().type('!@#$%^&*()')//input job title
      cy.get(':nth-child(2) > .oxd-input-group > :nth-child(2) > .oxd-textarea').clear().type(`!@#$%^&*()`)//input job deskripsi
      cy.get('.oxd-file-div').attachFile(file)//input job specification
      cy.get(':nth-child(4) > .oxd-input-group > :nth-child(2) > .oxd-textarea').clear().type(`!@#$%^&*()`)//input note
      cy.get('.oxd-button--secondary').click()//save

      cy.wait(2000)
      cy.get('.orangehrm-container').contains('!@#$%^&*()').should('exist')
    })
  })