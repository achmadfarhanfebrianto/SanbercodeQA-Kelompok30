...
//Add job title with special characters in all field
describe('failed_addjobtitle', () => {
    it('failed_addjobtitle', () => {
      ...
      cy.get(':nth-child(2) > .oxd-input').type('!@#$%^&*()')//input job title
      cy.get(':nth-child(2) > .oxd-input-group > :nth-child(2) > .oxd-textarea').type(`!@#$%^&*()`)//input job deskripsi
      cy.get('.oxd-file-div').attachFile(file)//input job specification
      cy.get(':nth-child(4) > .oxd-input-group > :nth-child(2) > .oxd-textarea').type(`!@#$%^&*()`)//input note
